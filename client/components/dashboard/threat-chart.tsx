    ...
    return (
        <div className="bg-card border border-border rounded-xl lg:rounded-2xl overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 lg:px-5 py-3 lg:py-4 border-b border-border gap-3">
                ...
                <div className="flex items-center gap-2">
                    <button className="text-xs px-2 lg:px-2.5 py-1 rounded-md bg-primary/10 text-primary font-medium">24h</button>
                    <button className="text-xs px-2 lg:px-2.5 py-1 rounded-md text-muted-foreground hover:text-foreground transition-colors">7d</button>
                    <button className="text-xs px-2 lg:px-2.5 py-1 rounded-md text-muted-foreground hover:text-foreground transition-colors">30d</button>
                </div>
            </div>
            ...
            <div className="px-4 lg:px-5 pt-4 lg:pt-5 pb-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 mb-4">
                    <div>
                        <p className="text-lg lg:text-xl font-bold font-display text-foreground">89 threats</p>
                ...
                <div className="relative">
                    <div className="flex items-end gap-0.5 h-16 lg:h-20">
                    ...
                    <div className="flex mt-1">
                        {hours.map((h, i) => (
                            <div key={i} className="flex-1 text-center">
                                <span className="text-[8px] lg:text-[9px] text-muted-foreground/60 font-mono">{h}</span>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        {weekDays.map((day, i) => (
                            <div key={day} className="flex-1 flex flex-col items-center gap-1">
                                <motion.div
                                    ...
                                    style={{ height: `${(weeklyAlerts[i] / weeklyMax) * 24}px` }}
                                    initial={{ height: 0 }}
                                    animate={{ height: `${(weeklyAlerts[i] / weeklyMax) * 24}px` }}
                                    transition={{ delay: 0.5 + i * 0.06, duration: 0.4 }}
                                />
                                <span className="text-[8px] lg:text-[9px] text-muted-foreground font-medium">{day}</span>
                            </div>
                        ))}
                    </div>
                </div>
