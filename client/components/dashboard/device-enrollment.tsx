...
export function DeviceEnrollment() {
  ...
  const [showTokenSecret, setShowTokenSecret] = useState<
          ...
          {status === "loading" ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Create a token to enroll new devices
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {tokens.map((token) => {
                const tokenSecret = getTokenSecret(token.id);
                const isShowingSecret = showTokenSecret[token.id];
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                copyToClipboard(tokenSecret, "Enrollment token")
                              }
                              className="gap-1 h-6"
                            >
                          ...
                          <p className="text-xs text-amber-600 mt-2">
                            ⚠️ This secret token is only shown once at creation.
                            Store it securely for agent enrollment.
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
