...
const mockConnections: NetworkConnection[] = [
    ...
    { source: "srv-db-01", target: "ws-finance-01", strength: 0.7, encrypted: true },
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-green-500" /><span>Online</span></div>
                    <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-amber-500" /><span>Warning</span></div>
                    <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500" /><span>Offline</span></div>
                </div>
            </div>

            <div className="p-5">
                <svg ref={svgRef} width="800" height="500" viewBox="0 0 800 500" className="w-full h-full">
                    <g>
                        ...
                        {mockNodes.map((node) => {
                            ...
                            return (
                                <g key={node.id}>
                                    ...
                                    <text x={node.x} y={node.y + 35} fontSize="12" fill="currentColor" textAnchor="middle" className="font-medium">
                                        {node.name}
                                    </text>
                                </g>
                            );
                        ...
                        >
                            <div className="flex items-start justify-between mb-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">ID: {selectedNode.id} • Type: {selectedNode.type}</p>
                                </div>
                                <div className="flex items-center gap-1">
                                    <Activity className="h-3 w-3 text-green-500" />
                                    <span className="text-xs text-green-500">{selectedNode.status}</span>
                                </div>
                            </div>
