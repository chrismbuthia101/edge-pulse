...
export class NotificationRepository extends BaseRepository<NotificationRow> {
  ...
  async findNotifications(options: NotificationQueryOptions = {}): Promise<NotificationRow[]> {
    const cacheKey = options.cacheKey ?? `notifications_${JSON.stringify(options)}`;
  ...
  async findNotificationsPaginated(
    ...
    return this.findPaginated({
      ...
      filters,
      orderBy: queryOptions.orderBy ?? { column: 'created_at', ascending: false },
      cacheTTL: queryOptions.cacheTTL,
    });
  }

  async getUnreadNotifications(userId: string, organizationId: string): Promise<NotificationRow[]> {
  ...
  async markAsRead(id: string): Promise<NotificationRow> {
    return this.update(id, { read: true, read_at: new Date().toISOString() } as Partial<NotificationRow>);
  }
  ...
  subscribeToNotifications(
    ...
    this.subscribe(channelName, filters, (payload) => {
      try {
        const p = payload as { eventType: string; new?: NotificationRow; old?: NotificationRow };
        switch (p.eventType) {
