...
export class DeviceDataRepository extends BaseRepository {
  ...
  async purgeDeviceData(options: PurgeOptions): Promise<PurgeResult[]> {
    ...
    try {
      if (options.purgeType === "telemetry") {
        const result = await this.purgeTelemetryData(
          options.deviceId,
          options.cutoffDate,
        );
        results.push(result);
      } else if (options.purgeType === "alerts") {
        const result = await this.purgeAlertData(
          options.deviceId,
          options.cutoffDate,
        );
        results.push(result);
        ...
        for (const table of tables) {
          const result = await this.purgeTableData(
            table,
            options.deviceId,
            options.cutoffDate,
          );
  ...
  private async purgeTelemetryData(
    deviceId: string,
    cutoffDate: string,
  ): Promise<PurgeResult> {
  ...
  private async purgeTableData(
    ...
    const client =
      schema === "public" ? this.supabase : this.supabase.schema(schema);
    ...
    let deleteQuery = client.from(table).delete().eq("device_id", deviceId);
