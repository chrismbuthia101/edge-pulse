...
import { LiveRepository } from "@/lib/repositories/live-repository";
...
function getLiveService(): LiveService {
  if (!liveService) {
    liveService = new LiveService(new LiveRepository(createClient()));
  }
  return liveService;
}
...
type LiveStore = typeof initialState & {
...
export const useLiveStore = create<LiveStore>()(
  devtools(
    (set, get) => ({
      ...
      initialize: async (supabaseClient) => {
        const client = supabaseClient ?? createClient();
        liveService = new LiveService(new LiveRepository(client));

        set({ status: "loading" });

        const initResult = await getLiveService().initializeLiveFeed();
        if (!initResult.success) {
          set({ error: initResult.error, status: "error" });
          return;
        }

        const alertEvents = initResult.data.alerts.map(alertToLiveEvent);
        const telemetryEvents = initResult.data.telemetry
          .filter((t) => t.source === "RESOURCE")
        ...
        set({
          events: allEvents,
          todayStats: initResult.data.stats,
          status: "success",
        });

        const subResult = getLiveService().subscribeToLiveFeed({
          onNewAlert: (alert) => {
            if (!get().paused) {
      ...
      addAlertEvent: (alert) => {
        ...
        set((state) => ({
          ...
          todayStats: {
            ...
            critical:
              state.todayStats.critical +
              (alert.severity === "critical" ? 1 : 0),
          },
        }));
      },
