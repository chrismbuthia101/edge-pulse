...
export function useAlerts() {
    ...
    const filtered = useMemo<Alert[]>(() => {
        switch (filter) {
            case "PENDING": return alerts.filter((a) => a.status === "PENDING");
            case "IN_REVIEW": return alerts.filter((a) => a.status === "ACKNOWLEDGED" || a.status === "INVESTIGATED");
            case "CLOSED": return alerts.filter((a) => a.status === "CLOSED");
            default: return alerts.filter((a) => a.status !== "CLOSED");
        }
    }, [alerts, filter]);
    ...
    const handleResolve = useCallback(
        async (e: React.MouseEvent, alertId: string, currentStatus: AlertStatus) => {
            e.stopPropagation();
