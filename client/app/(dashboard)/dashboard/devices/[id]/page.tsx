...
export default function DeviceDetailPage() {
    ...
    const fetchResilienceData = async () => {
        ...
        try {
            const metrics = await resilienceService.getConnectionMetrics({ timeRange: '1h' as const });
            const deviceMetrics = metrics.find((m: ConnectionMetrics) => m.device_id === deviceId);
                ...
                <TabsContent value="health" className="space-y-6">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="p-4 bg-muted/30 rounded-lg">
                                <p className="text-sm text-muted-foreground mb-2">Agent Version</p>
                                <div className="flex items-center gap-2">
                                    <Zap className="h-4 w-4 text-amber-500" />
                                    <span className="text-2xl font-bold">{agentVersion}</span>
                                </div>
                                <p className="text-xs text-muted-foreground mt-2">
                                    {agentVersion === "v2.4.1" ? "Up to date" : "Update available"}
                                </p>
                            </div>
                            ...
                            <div className="p-4 bg-muted/30 rounded-lg">
                                ...
                                <div className="flex items-center gap-2">
                                    {hashChainValid ? <Link2 className="h-4 w-4 text-green-500" /> : <Link2Off className="h-4 w-4 text-destructive" />}
                                    <span className="text-2xl font-bold">{hashChainValid ? "Intact" : "Broken"}</span>
                                </div>
                                <p className="text-xs text-muted-foreground mt-2">
                                    {hashChainValid ? "Data integrity verified" : "Integrity check failed"}
                                </p>
                            </div>
                            ...
                            <div className="p-4 bg-muted/30 rounded-lg">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-2xl font-bold">{relativeTime(device.last_seen)}</span>
                                </div>
                                <p className="text-xs text-muted-foreground mt-2">
                ...
                <TabsContent value="network" className="space-y-6">
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <Button variant="outline" size="sm" onClick={fetchResilienceData} disabled={resilienceLoading}>
                                <RefreshCw className={cn("h-4 w-4 mr-2", resilienceLoading && "animate-spin")} />
                                Refresh
                            </Button>
                        </div>
                        ...
                        {resilienceLoading ? (
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Connection State</p>
                                        <Badge className={cn(
                                            "text-xs",
                                            connectionMetrics.connection_state === "ONLINE" ? "bg-green-500/10 text-green-500 border-green-500/20" :
                                                connectionMetrics.connection_state === "DEGRADED" ? "bg-amber-500/10 text-amber-500 border-amber-500/20" :
                                    ...
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Signal className={cn("h-4 w-4", connectionMetrics.signal_strength >= 80 ? "text-green-500" : connectionMetrics.signal_strength >= 60 ? "text-amber-500" : "text-red-500")} />
                                            <span className="text-2xl font-bold">{connectionMetrics.signal_strength}%</span>
                                        </div>
                                        <Progress value={connectionMetrics.signal_strength} className="mt-2" />
                                    </div>

                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Latency</p>
                                        <div className="flex items-center gap-2">
                                            <Clock className={cn("h-4 w-4", connectionMetrics.latency_ms < 50 ? "text-green-500" : connectionMetrics.latency_ms < 100 ? "text-amber-500" : "text-red-500")} />
                                            <span className="text-2xl font-bold">{connectionMetrics.latency_ms}ms</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <TrendingDown className={cn("h-4 w-4", connectionMetrics.packet_loss > 2 ? "text-red-500" : "text-green-500")} />
                                            <span className="text-2xl font-bold">{connectionMetrics.packet_loss.toFixed(1)}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Bandwidth (Down)</p>
                                        <div className="flex items-center gap-2">
                                            <Download className="h-4 w-4 text-blue-500" />
                                            <span className="text-2xl font-bold">{Math.round(connectionMetrics.bandwidth_down)} Mbps</span>
                                        </div>
                                    </div>

                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Bandwidth (Up)</p>
                                        <div className="flex items-center gap-2">
                                            <Upload className="h-4 w-4 text-green-500" />
                                            <span className="text-2xl font-bold">{Math.round(connectionMetrics.bandwidth_up)} Mbps</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Battery className={cn("h-4 w-4", connectionMetrics.uptime_percentage > 95 ? "text-green-500" : "text-amber-500")} />
                                            <span className="text-2xl font-bold">{connectionMetrics.uptime_percentage.toFixed(1)}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Queue Depth</p>
                                        <div className="flex items-center gap-2">
                                            <HardDrive className="h-4 w-4 text-orange-500" />
                                            <span className="text-2xl font-bold">{connectionMetrics.queue_depth}</span>
                                        </div>
                                        <p className="text-xs text-muted-foreground mt-2">
                                            {connectionMetrics.queue_depth > 0 ? "Items pending sync" : "No pending items"}
                                        </p>
                                    </div>

                                    <div className="p-4 bg-muted/30 rounded-lg">
                                        <p className="text-sm text-muted-foreground mb-2">Reconnect Attempts</p>
                                        <div className="flex items-center gap-2">
                                            <RefreshCw className="h-4 w-4 text-primary" />
                                            <span className="text-2xl font-bold">{connectionMetrics.reconnect_attempts}</span>
                                        </div>
                                        <p className="text-xs text-muted-foreground mt-2">
                                            {connectionMetrics.reconnect_attempts > 0 ? "Connection attempts made" : "No reconnection needed"}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="text-center py-12 text-muted-foreground">
                                ...
                                <p className="text-sm mt-2">Click refresh to load network metrics</p>
                            </div>
                        )}
                    </motion.div>
                </TabsContent>
