...
const deviceStateConfig: Record<
  DeviceState,
          ...
          <div className="space-y-4">
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 text-sm">
              ...
              <p className="text-muted-foreground text-xs">
                This token is only shown once. Copy it now before proceeding.
              </p>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-border">
              ...
              <div className="bg-muted rounded-lg p-3 font-mono text-xs">
                <p className="text-muted-foreground mb-2">
                  # Download the .deb package from your dashboard or build from
                  source
                </p>
                <p className="text-foreground">
                  sudo dpkg -i edgepulse-agent_VERSION_amd64.deb
                </p>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-muted rounded-lg p-3 mt-2">
                <p className="text-xs text-muted-foreground mb-2">
                  Or for automated deployment, create enrollment.json:
                </p>
                <pre className="font-mono text-xs bg-card p-3 rounded overflow-x-auto">
                  {`{
        ...
        {step === "enroll" && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-muted rounded-lg p-3">
                <p className="text-xs text-muted-foreground mb-2">
                  Start the EdgePulse agent service:
                </p>
                <div className="flex gap-2 items-center">
                  <code className="font-mono text-xs bg-card p-3 rounded flex-1">
...
export default function DevicesPage() {
  ...
  const [isolateDevice, setIsolateDevice] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const enrichedDevices = useMemo(
    () =>
      rawDevices.map((d) => ({
        ...d,
        deviceState: computeDeviceState(
          d.status,
          d.lastSeenIso ?? undefined,
          d.syncQueueDepth,
        ),
      })),
    [rawDevices],
  ...
  const filtered = useMemo(() => {
    ...
    let result = enrichedDevices.filter((d) => {
      ...
      const matchesStatus =
        statusFilter === "all" || d.deviceState === statusFilter;
    ...
    if (sortKey) {
      result = [...result].sort((a, b) => {
        ...
        if (sortKey === "risk") {
          aVal = riskOrder[a.risk as keyof typeof riskOrder] ?? 99;
          bVal = riskOrder[b.risk as keyof typeof riskOrder] ?? 99;
        }
        if (sortKey === "state") {
          aVal = a.deviceState;
          bVal = b.deviceState;
        ...
        if (sortKey === "syncQueue") {
          aVal = a.syncQueueDepth;
          bVal = b.syncQueueDepth;
        }
  ...
  const counts = useMemo(
    () => ({
      ...
      reporting: enrichedDevices.filter((d) => d.deviceState === "reporting")
        .length,
      silent: enrichedDevices.filter((d) => d.deviceState === "silent").length,
      unsynced: enrichedDevices.filter((d) => d.deviceState === "unsynced")
        .length,
      atRisk: enrichedDevices.filter((d) =>
        ["critical", "high"].includes(d.risk),
      ).length,
    }),
    [enrichedDevices],
  );
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {counts.reporting} reporting · {counts.silent} silent ·{" "}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-muted/50 rounded-lg p-0.5">
            {(
              ["all", "reporting", "silent", "unsynced", "offline"] as const
            ).map((f) => (
              <button
                key={f}
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                key && "hover:text-foreground transition-colors cursor-pointer",
              )}
            >
              {label}
        ...
        <div className="divide-y divide-border">
          {filtered.map((device, i) => {
            const risk =
              riskConfig[device.risk as keyof typeof riskConfig] ??
              riskConfig.none;
            ...
            return (
              ...
              >
                {/* Mobile card */}
                <div
                  className="lg:hidden p-4 space-y-3 hover:bg-muted/30 cursor-pointer transition-colors"
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="space-y-1">
                      ...
                      <p className={cn("font-semibold", state.color)}>
                        {state.label}
                      </p>
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <MiniBar
                          value={device.cpu}
                          color={
                            device.cpu > 80 ? "bg-destructive" : "bg-primary"
                          }
                        />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <MiniBar
                          value={device.mem}
                          color={
                            device.mem > 80 ? "bg-orange-500" : "bg-violet-500"
                          }
                        />
                        <span className="text-[10px] font-mono text-muted-foreground">
                          {device.mem}%
                  ...
                  >
                    <div className="pt-4 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                      <div>
                        ...
                        <p className={cn("text-sm font-semibold", state.color)}>
                          {state.label}
                        </p>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2 mt-4 flex-wrap">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setIsolateDevice({ id: device.id, name: device.name })
                        }
                      >
                        Isolate
                      </Button>
        ...
        <div className="px-5 py-3 border-t border-border">
          <p className="text-xs text-muted-foreground">
            {enrichedDevices.length > 0
              ? `Showing ${filtered.length} of ${enrichedDevices.length} devices`
              : "No devices enrolled"}
          </p>
        </div>
      </div>
