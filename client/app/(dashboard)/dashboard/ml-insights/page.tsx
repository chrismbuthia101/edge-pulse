...
import type { ShapFeature } from "@/lib/types/alerts";
...
export default function InsightsPage() {
  ...
  useEffect(() => {
    if (!initializedRef.current && hasRole(["ORG_ADMIN"])) {
    ...
    const loadInsightsData = async () => {
      try {
        ...
        const highConfidenceAlerts = alerts
          ...
          .map((a) => ({
            device:
              devices.find((d) => d.id === a.device_id)?.name ?? a.device_id,
        ...
        if (devices.length > 0) {
          ...
          await useAnomalyStore.getState().refreshLatestScore(firstDeviceId);
          const anomalyStore = useAnomalyStore.getState();
          const latestScore = anomalyStore.latestScore;

          const healthDevices = useHealthStore.getState().devices;
          const deviceHealth = healthDevices.find(
            (d) => d.device_id === firstDeviceId,
          );

          if (latestScore) {
            setRealTimeMetrics({
              currentScore: latestScore.score,
              cpuUsage: deviceHealth?.cpu_usage ?? 0,
              memoryUsage: deviceHealth?.memory_usage ?? 0,
              featureVectorSize: 47,
              lastInference: formatTimeAgo(latestScore.scored_at),
              inferencesPerSecond: 2.4,
              anomalyThreshold: latestScore.threshold_applied,
            });
          }
          ...
          if (
            recentAlertsWithShap.length > 0 &&
            recentAlertsWithShap[0].explanation_json?.features
          ) {
            const shapFeatures =
              recentAlertsWithShap[0].explanation_json.features
                .slice(0, 6)
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              ML Insights are available to administrators only.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Model performance, explainability, and detection analysis
          </p>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-1.5">
                    <p className="text-sm text-foreground w-48 shrink-0 font-medium">
                      {feat.label}
                    </p>
                    ...
                    <span
                      className={`text-sm font-mono font-bold w-12 text-right ${feat.positive ? "text-destructive" : "text-primary"}`}
                    >
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 mb-2">
                <div
                  className={`w-3 h-3 rounded-full ${realTimeMetrics.currentScore && realTimeMetrics.anomalyThreshold && realTimeMetrics.currentScore > realTimeMetrics.anomalyThreshold ? "bg-red-500" : "bg-green-500"} animate-pulse`}
                />
                <span
                  className={`text-xl font-bold font-display ${realTimeMetrics.currentScore && realTimeMetrics.anomalyThreshold && realTimeMetrics.currentScore > realTimeMetrics.anomalyThreshold ? "text-red-500" : "text-green-500"}`}
                >
                  {realTimeMetrics.currentScore
              ...
              <p className="text-[9px] text-muted-foreground">
                Threshold: {realTimeMetrics.anomalyThreshold || "0.75"}
              </p>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 mb-2">
            ...
            <div className="text-center">
              ...
              <p className="text-[9px] text-violet-500">
                Last: {realTimeMetrics.lastInference || "Never"}
              </p>
            </div>
          </div>
          <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span>
                Feature Vector: {realTimeMetrics.featureVectorSize || 0}{" "}
                dimensions
              </span>
              <span>·</span>
              ...
              <span>
                Status:{" "}
                {realTimeMetrics.currentScore &&
                realTimeMetrics.anomalyThreshold &&
                realTimeMetrics.currentScore > realTimeMetrics.anomalyThreshold
                  ? "ANOMALY DETECTED"
                  : "Normal Operation"}
              </span>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm font-mono font-bold ${det.score > 0.9 ? "text-destructive" : "text-orange-500"}`}
                  >
                    {det.score.toFixed(2)}
                  </span>
                </div>
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full ${det.blocked ? "bg-green-500/10 text-green-500" : "bg-amber-500/10 text-amber-500"}`}
                >
