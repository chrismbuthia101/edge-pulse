...
function StatCard({
  ...
  return (
    ...
    >
      {/* Subtle glow accent */}
      <div
        className={`absolute -top-6 -right-6 w-24 h-24 rounded-full ${accentBg} blur-2xl opacity-60 pointer-events-none`}
      />

      <div className="relative">
        <div className="flex items-start justify-between mb-4">
          <div
            className={`w-10 h-10 rounded-xl border flex items-center justify-center ${accentBg} ${accentBorder}`}
          >
            <Icon className={`h-5 w-5 ${accent}`} />
          </div>
          <span
            className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-full ${deltaPositive ? "bg-green-500/10 text-green-500" : "bg-destructive/10 text-destructive"}`}
          >
            {deltaPositive ? (
...
export default function DashboardPage() {
  ...
  const anomaliesResolved = useMemo(
    () => alerts.filter((a) => a.status === "CLOSED").length,
    [alerts],
  );
  ...
  const latencyAlerts = useMemo(
    () => alerts.filter((a) => a.inference_latency_ms > 0).slice(0, 50),
    [alerts],
  );
  const avgLatency = useMemo(
    () =>
      latencyAlerts.length > 0
        ? Math.round(
            latencyAlerts.reduce(
              (s: number, a: Alert) => s + a.inference_latency_ms,
              0,
            ) / latencyAlerts.length,
          )
        : 0,
    [latencyAlerts],
  );
  ...
  const radialData = useMemo(
    () => [{ name: "Rate", value: responseRate, fill: "#06b6d4" }],
    [responseRate],
  );
                  ...
                  >
                    {sourceBreakdown.map((_, i) => (
                      <Cell
                        key={i}
                        fill={
                          ["#06b6d4", "#8b5cf6", "#f59e0b", "#ef4444"][i % 4]
                        }
                      />
                    ))}
                  </Bar>
        ...
        >
          ...
          <div className="p-5 space-y-3">
            {deviceRisk.length > 0 ? (
              deviceRisk.map((d) => {
                const pct =
                  devices.length > 0 ? (d.value / devices.length) * 100 : 0;
                return (
                  <div key={d.name}>
              ...
              {recentCritical.length === 0 ? (
                ...
                recentCritical.map((alert, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-1.5 h-8 rounded-full shrink-0 ${isCritical ? "bg-destructive shadow-[0_0_8px_#ef4444]" : "bg-orange-500"}`}
                      />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                          <span className="truncate">{alert.device_id}</span>
                          <span>·</span>
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isCritical ? "bg-destructive/15 text-destructive" : "bg-orange-500/15 text-orange-500"}`}
                        >
                          {alert.severity.toUpperCase()}
                        </span>
                        <span className="text-[10px] font-mono text-muted-foreground">
                          {(
                            (alert.anomaly_score ?? alert.confidence ?? 0) * 100
                          ).toFixed(0)}
                          %
                        </span>
                      </div>
          ...
          <div className="px-5 py-3 border-t border-border bg-muted/20 flex items-center justify-between">
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1">
                ...
                {
                  alerts.filter(
                    (a) => a.severity === "high" && a.status !== "CLOSED",
                  ).length
                }{" "}
                high
              </span>
