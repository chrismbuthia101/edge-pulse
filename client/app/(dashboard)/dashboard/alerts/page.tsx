...
function AlertRulesModal({
  ...
  const rules = [
    {
      name: "Critical Process Injection",
      condition: "anomaly_score > 0.95 AND source = PROCESS",
      action: "Auto-block + Alert",
      active: true,
    },
    {
      name: "Outbound Data Exfiltration",
      condition: "network_bytes_out > 100MB in 5min",
      action: "Alert + Quarantine",
      active: true,
    },
    {
      name: "Brute Force Detection",
      condition: "auth_failures > 50 in 60s",
      action: "Block IP + Alert",
      active: true,
    },
    {
      name: "Off-Hours Login",
      condition: "login_hour NOT IN (8-18) AND is_admin = true",
      action: "Alert only",
      active: false,
    },
  ];
...
export default function AlertsPage() {
    ...
    return alerts.filter((a) => {
      const matchesStatus =
        ...
        (statusFilter === "IN_REVIEW" &&
          (a.status === "ACKNOWLEDGED" || a.status === "INVESTIGATED")) ||
      ...
      const matchesSearch =
        ...
        (devices
          ...
          false);
      const matchesDevice =
        selectedDevice === "ALL" || a.device_id === selectedDevice;
      return matchesStatus && matchesSeverity && matchesSearch && matchesDevice;
    });
  }, [alerts, devices, statusFilter, severityFilter, search, selectedDevice]);
  ...
  const handleMarkAllReviewed = async () => {
    ...
    if (pending.length === 0) {
      toast.info("No pending alerts to review");
      return;
    }
  ...
  const getDeviceName = (alert: (typeof alerts)[0]) => {
    ...
    return device?.name || alert.device_id || "Unknown Device";
  };
  return (
    <div className="max-w-300 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div
            key={s.label}
            className={`${s.bg} rounded-xl border border-border p-4`}
          >
                      ...
                      >
                        <div className="mt-4 pt-4 border-t border-border space-y-4">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                            ...
                            {alert.telemetry_source === "PROCESS" &&
                              alert.proc_privilege_level && (
                                <div>
                                  <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">
                                    Privilege
                                  </p>
