...
const reportCards: ReportCard[] = [
  {
    ...
    description:
      "Deep-dive into security alerts by severity, category, device, and time range with exportable data.",
    icon: AlertTriangle,
    color: "text-destructive",
    bg: "bg-destructive/10",
  ...
  },
  {
    ...
    description:
      "Fleet-wide device health, risk distribution, performance metrics, and agent compliance.",
  ...
  },
  {
    ...
    description:
      "High-level security posture overview with KPIs, trends, and executive-ready visualizations.",
    icon: BarChart3,
    color: "text-violet-500",
    bg: "bg-violet-500/10",
    ...
    getStats: (alerts) => [
      ...
      {
        label: "Resolved",
        value: alerts.filter((a) => a.status === "CLOSED").length.toString(),
      },
    ],
  },
  {
    ...
    description:
      "AI model accuracy, precision, recall, inference latency, and SHAP explainability metrics.",
    icon: Brain,
    ...
    getStats: (alerts) => [
      {
        ...
        value:
          alerts.length > 0
            ? `${Math.round(alerts.reduce((s, a) => s + a.inference_latency_ms, 0) / alerts.length)}ms`
            : "—",
      },
  ...
  },
  {
    ...
    description:
      "Hash chain verification status, tamper detection alerts, and tamper-evident log analysis.",
    icon: Shield,
    color: "text-sky-500",
    bg: "bg-sky-500/10",
    ...
    getStats: (_, devices) => [
      { label: "Devices", value: devices.length.toString() },
      {
        label: "Verified",
        value: devices
          .filter((d) => d.hash_chain_ok !== false)
          .length.toString(),
      },
    ],
  },
  {
    ...
    description:
      "Complete user roster with role distribution, approval status, and account activity metrics.",
    icon: Users,
...
export default function ReportsPage() {
  ...
  const exportHubSummary = () => {
    const lines = [
      ...
      "AVAILABLE REPORTS",
      ...filteredReports.map(
        (r) => `• ${r.title} (${r.category}) - ${r.roles.join(", ")}`,
      ),
      "",
  ...
  const hasAnyReportAccess = reportCards.some((report) =>
    hasRole(report.roles),
  );
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                bgMap[s.color] || "bg-card",
                borderMap[s.color] || "border-border",
              )}
            >
              <div
                className={cn(
              ...
              <p className="text-xs text-muted-foreground mt-0.5 relative">
                {s.label}
              </p>
            </motion.div>
          );
              ...
              {cards.map((card, i) => (
                <Link key={card.id} href={card.href}>
                  ...
                  >
                    <div
                      className={cn(
                        "absolute -top-6 -right-6 w-24 h-24 rounded-full opacity-5",
                        card.bg,
                      )}
                    />
                    ...
                    <h3 className="text-base font-semibold text-foreground mb-2 relative">
                      {card.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-6 pt-3 border-t border-border/50 mb-3 relative">
                      {card.getStats &&
                        card
                          .getStats(filteredAlerts, filteredDevices)
                          .slice(0, 2)
                          .map((stat, idx) => (
                            <div key={idx}>
                              <p
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Executive Summary and ML Performance reports require administrator
              access.
            </p>
          </div>
        </motion.div>
