...
import {
    ArrowLeft, Download, RefreshCw, Filter, Search, Shield, AlertTriangle, CheckCircle, XCircle,
...
export default function IntegrityAuditReport() {
    useEffect(() => { document.title = "Integrity Audit Report - EdgePulse"; }, []);
    ...
    useEffect(() => {
        if (!initialized.current) { initialized.current = true; initDevices(); initAlerts(); }
    ...
    const filteredDevices = useMemo(() => {
        return devices.filter(d => {
            const integrityStatus = d.hash_chain_ok === true ? "verified" : d.hash_chain_ok === false ? "tampered" : "unknown";
            if (integrity !== "all" && integrityStatus !== integrity) return false;
            if (risk !== "all" && d.risk !== risk) return false;
            if (status !== "all" && d.status !== status) return false;
            if (search && !d.name.toLowerCase().includes(search.toLowerCase())) return false;
            return true;
        });
    }, [devices, integrity, risk, status, search]);

    const tamperAlerts = useMemo(() => {
        return alerts.filter(a => {
            if (dateRange !== "all" && new Date(a.created_at) < cutoff) return false;
            return a.category === "TAMPER" || a.title.toLowerCase().includes("tamper") || a.title.toLowerCase().includes("integrity");
        });
    }, [alerts, dateRange, cutoff]);

    const metrics = useMemo(() => ({
        totalDevices: filteredDevices.length,
        verified: filteredDevices.filter(d => d.hash_chain_ok === true).length,
        tampered: filteredDevices.filter(d => d.hash_chain_ok === false).length,
        unknown: filteredDevices.filter(d => d.hash_chain_ok === null || d.hash_chain_ok === undefined).length,
        atRisk: filteredDevices.filter(d => d.risk === "critical" || d.risk === "high").length,
        tamperAlerts: tamperAlerts.length,
        verificationRate: filteredDevices.length > 0 ? Math.round((filteredDevices.filter(d => d.hash_chain_ok === true).length / filteredDevices.length) * 100) : 0,
    }), [filteredDevices, tamperAlerts]);

    const integrityDist = useMemo(() => {
        const counts: Record<string, number> = { verified: 0, tampered: 0, unknown: 0 };
        filteredDevices.forEach(d => {
            if (d.hash_chain_ok === true) counts.verified++;
            else if (d.hash_chain_ok === false) counts.tampered++;
            else counts.unknown++;
        });
        return Object.entries(counts).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value, color: INTEGRITY_COLORS[name] }));
    }, [filteredDevices]);

    const riskDist = useMemo(() => {
        const counts: Record<string, number> = { critical: 0, high: 0, medium: 0, low: 0 };
        filteredDevices.forEach(d => { if (d.risk && counts[d.risk] !== undefined) counts[d.risk]++; });
    ...
    const devicesByRisk = useMemo(() => {
        const counts: Record<string, { name: string; count: number; tampered: number }> = {};
        filteredDevices.forEach(d => {
            const riskLevel = d.risk || "unknown";
            if (!counts[riskLevel]) counts[riskLevel] = { name: riskLevel.charAt(0).toUpperCase() + riskLevel.slice(1), count: 0, tampered: 0 };
            counts[riskLevel].count++;
            if (d.hash_chain_ok === false) counts[riskLevel].tampered++;
        });
    ...
    const exportCSV = () => {
        setExporting(true);
        const rows = [
            ["Device ID", "Name", "Hash Chain Status", "Risk Level", "Status", "CPU %", "RAM %", "Sync Queue", "Last Seen"],
        ...
        <div className="max-w-[1200px] space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Select value={integrity} onValueChange={(v) => setIntegrity(v as IntegrityFilter)}>
                    ...
                    <Select value={status} onValueChange={(v) => setStatus(v as StatusFilter)}>
                        ...
                        <SelectContent>
                            ...
                            {["online", "offline", "isolated"].map(s => <SelectItem key={s} value={s} className="capitalize">{s}</SelectItem>)}
                        </SelectContent>
                    </Select>
                    <div className="relative flex-1 min-w-[160px]">
                        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
                        <Input placeholder="Search devices..." className="pl-8 h-8 text-xs" value={search} onChange={e => setSearch(e.target.value)} />
                    </div>
                </div>
            </motion.div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
                {[
                    { label: "Total Devices", value: metrics.totalDevices, color: "text-foreground" },
                    { label: "Verified", value: metrics.verified, color: "text-green-500" },
                    { label: "Tampered", value: metrics.tampered, color: "text-destructive" },
                    { label: "Unknown", value: metrics.unknown, color: "text-amber-500" },
                    { label: "At Risk", value: metrics.atRisk, color: "text-orange-500" },
                    { label: "Tamper Alerts", value: metrics.tamperAlerts, color: "text-destructive" },
                    { label: "Verification Rate", value: `${metrics.verificationRate}%`, color: "text-sky-500" },
                ].map((s, i) => (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                        ...
                        <span className="text-xs text-muted-foreground">{tamperAlerts.length} alerts</span>
                    </div>
                    <div className="p-4 h-56">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={tamperTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                                <defs>
                                ...
                                <Area type="monotone" dataKey="tamperAlerts" name="Tamper Alerts" stroke="#ef4444" strokeWidth={1.5} fill="url(#tamperGradient)" dot={false} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                    ...
                    <div className="px-5 py-4 border-b border-border">
                        <h3 className="text-sm font-semibold text-foreground">Integrity Distribution</h3>
                    </div>
                        ...
                        {integrityDist.length > 0 ? (
                            <>
                                <div className="flex-1 h-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={integrityDist} cx="50%" cy="50%" innerRadius="50%" outerRadius="80%" paddingAngle={3} dataKey="value" strokeWidth={0}>
                                                {integrityDist.map((e, i) => <Cell key={i} fill={e.color} />)}
                                            </Pie>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    className="bg-card border border-border rounded-2xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-border">
                        <h3 className="text-sm font-semibold text-foreground">Devices by Risk Level</h3>
                    </div>
                    <div className="p-4 h-52">
                        {devicesByRisk.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={devicesByRisk} layout="vertical" margin={{ left: 0, right: 16 }}>
                                    ...
                                    <Bar dataKey="count" name="Total" fill="#06b6d4" radius={[0, 4, 4, 0]} maxBarSize={16} />
                                    <Bar dataKey="tampered" name="Tampered" fill="#ef4444" radius={[0, 4, 4, 0]} maxBarSize={16} />
                                </BarChart>
                            </ResponsiveContainer>
                        ) : (
                        ...
                        {riskDist.length > 0 ? (
                            <>
                                <div className="flex-1 h-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={riskDist} cx="50%" cy="50%" innerRadius="45%" outerRadius="75%" paddingAngle={3} dataKey="value" strokeWidth={0}>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                className="bg-card border border-border rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                    <h3 className="text-sm font-semibold text-foreground">Device Integrity Status</h3>
                    <span className="text-xs text-muted-foreground">Showing {filteredDevices.length} devices</span>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        <thead>
                            <tr className="border-b border-border bg-muted/30">
                                {["Device", "Hash Chain", "Risk", "Status", "CPU", "RAM", "Sync Queue", "Last Seen"].map(h => (
                            ...
                            {filteredDevices.map(d => {
                                ...
                                return (
                                    <tr key={d.id} className="hover:bg-muted/30 transition-colors">
                                        <td className="px-4 py-2.5 font-medium text-foreground max-w-[180px] truncate">{d.name}</td>
                                        <td className="px-4 py-2.5">
                                            <span className={cn("px-1.5 py-0.5 rounded text-[10px] font-bold capitalize flex items-center gap-1 w-fit", integrityColors[integrityStatus])}>
                                                {integrityStatus === "verified" && <CheckCircle className="h-3 w-3" />}
                                                {integrityStatus === "tampered" && <XCircle className="h-3 w-3" />}
                                                {integrityStatus === "unknown" && <AlertTriangle className="h-3 w-3" />}
                                                {integrityStatus}
                                            </span>
                                        </td>
                                        <td className="px-4 py-2.5">
                                            {d.risk ? (
                                                <span className={cn("px-1.5 py-0.5 rounded text-[10px] font-bold capitalize", riskColors[d.risk])}>{d.risk}</span>
                                            ) : (
                                                <span className="text-muted-foreground">—</span>
                                            )}
                                        </td>
                                        <td className="px-4 py-2.5 text-muted-foreground capitalize">{d.status}</td>
                                        ...
                                        <td className="px-4 py-2.5 text-muted-foreground">{d.sync_queue_depth ?? "—"}</td>
                                        <td className="px-4 py-2.5 text-muted-foreground flex items-center gap-1">
