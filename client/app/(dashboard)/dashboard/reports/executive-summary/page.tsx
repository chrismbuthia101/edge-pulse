...
import {
    ...
    MonitorSmartphone, Lock, ShieldAlert,
} from "lucide-react";
import {
...
function StatCard({ title, value, delta, positive, icon: Icon, color, bg }: {
    ...
    return (
        <div className={cn("rounded-2xl border p-5 relative overflow-hidden", bg)}>
            <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full opacity-30" style={{ background: "currentColor" }} />
            <div className="flex items-start justify-between mb-3 relative">
                <div className={cn("w-9 h-9 rounded-xl border flex items-center justify-center", bg, `border-${color.split("-")[1]}-500/30`)}>
                    <Icon className={cn("h-4 w-4", color)} />
                </div>
                <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5",
                    positive ? "bg-green-500/10 text-green-500" : "bg-destructive/10 text-destructive")}>
                    {positive ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                    {delta}
                </span>
            </div>
            <p className={cn("text-3xl font-bold font-display relative", color)}>{value}</p>
...
export default function ExecutiveSummaryReport() {
    useEffect(() => { document.title = "Executive Summary - EdgePulse"; }, []);
    ...
    const periodAlerts = useMemo(() => alerts.filter(a => new Date(a.created_at) >= cutoff), [alerts, cutoff]);
    const prevCutoff = useMemo(() => { const d = new Date(cutoff); d.setDate(d.getDate() - days); return d; }, [cutoff, days]);
    const prevAlerts = useMemo(() => alerts.filter(a => {
    ...
    const sevDist = useMemo(() => {
        ...
        periodAlerts.forEach(a => { if (c[a.severity] !== undefined) c[a.severity]++; });
        const colors: Record<string, string> = { critical: "#ef4444", high: "#f97316", medium: "#f59e0b", low: "#06b6d4" };
        return Object.entries(c).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value, color: colors[name] }));
    }, [periodAlerts]);

    const threatsBySource = useMemo(() => {
        const c: Record<string, number> = {};
        periodAlerts.forEach(a => { const s = a.telemetry_source || "UNKNOWN"; c[s] = (c[s] || 0) + 1; });
        return Object.entries(c).sort((a, b) => b[1] - a[1]).map(([name, value]) => ({ name, value }));
    }, [periodAlerts]);

    const resolutionGauge = [{ name: "Rate", value: metrics.resRate, fill: metrics.resRate >= 80 ? "#22c55e" : metrics.resRate >= 50 ? "#f59e0b" : "#ef4444" }];
        ...
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground">This report is only available to administrators.</p>
                    <Button className="mt-4" variant="outline" onClick={() => router.push("/dashboard/reports")}>Back to Reports</Button>
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-[1200px] space-y-6">
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">Organization-wide security posture and threat intelligence</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" className="gap-1.5" onClick={exportReport} disabled={exporting}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
                    <StatCard title="Total Alerts" value={metrics.total.toString()}
                        delta={pct(metrics.total, metrics.prevTotal)} positive={trend(metrics.total, metrics.prevTotal)}
                        icon={ShieldAlert} color="text-destructive" bg="bg-destructive/5 border-destructive/15" />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}>
                    <StatCard title="Critical Threats" value={metrics.critical.toString()}
                        delta={`${metrics.critical} critical`} positive={metrics.critical === 0}
                        icon={AlertTriangle} color="text-orange-500" bg="bg-orange-500/5 border-orange-500/15" />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }}>
                    <StatCard title="Resolution Rate" value={`${metrics.resRate}%`}
                        delta={metrics.resRate >= 80 ? "On target" : "Below target"} positive={metrics.resRate >= 80}
                        icon={CheckCircle2} color="text-green-500" bg="bg-green-500/5 border-green-500/15" />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    <StatCard title="Devices Enrolled" value={devices.length.toString()}
                        delta={`${metrics.onlineDevices} online`} positive={metrics.onlineDevices > 0}
                        icon={MonitorSmartphone} color="text-primary" bg="bg-primary/5 border-primary/15" />
                </motion.div>
            </div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }}
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-destructive" />Detected</span>
                    ...
                    <div className="p-4 h-56">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={weeklyTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barGap={2}>
                                ...
                                <Bar dataKey="detected" name="Detected" fill="#ef4444" radius={[3, 3, 0, 0]} maxBarSize={16} />
                                <Bar dataKey="resolved" name="Resolved" fill="#22c55e" radius={[3, 3, 0, 0]} maxBarSize={16} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.26 }}
                    ...
                    <div className="flex flex-col items-center justify-center p-4 h-56">
                        <div className="relative w-full h-36">
                            <ResponsiveContainer width="100%" height="100%">
                                <RadialBarChart innerRadius="55%" outerRadius="90%" data={resolutionGauge} startAngle={210} endAngle={-30} barSize={14}>
                                    <RadialBar dataKey="value" cornerRadius={8} background={{ fill: "hsl(var(--muted))" }} isAnimationActive />
                                </RadialBarChart>
                            </ResponsiveContainer>
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                <span className={cn("text-3xl font-bold font-display",
                                    metrics.resRate >= 80 ? "text-green-500" : metrics.resRate >= 50 ? "text-amber-500" : "text-destructive")}>
                                    {metrics.resRate}%
                                </span>
                        ...
                        <div className="grid grid-cols-2 gap-3 w-full mt-1">
                            <div className="text-center">
                                <p className="text-lg font-bold text-foreground">{metrics.resolved}</p>
                                <p className="text-[10px] text-muted-foreground">Closed</p>
                            </div>
                            <div className="text-center">
                                <p className="text-lg font-bold text-destructive">{metrics.total - metrics.resolved}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        {sevDist.length > 0 ? (
                            <>
                                <div className="flex-1 h-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={sevDist} cx="50%" cy="50%" innerRadius="50%" outerRadius="80%" paddingAngle={3} dataKey="value" strokeWidth={0}>
                                ...
                                <div className="flex flex-col gap-2 pl-2">
                                    {sevDist.map(d => (
                                        <div key={d.name} className="flex items-center gap-2 text-xs">
                                            ...
                                            <span className="text-muted-foreground capitalize w-12">{d.name}</span>
                                            <span className="font-bold text-foreground">{d.value}</span>
                                        </div>
                                    ))}
                                </div>
                            </>
                        ) : <div className="flex-1 flex items-center justify-center"><p className="text-xs text-muted-foreground">No alerts in period</p></div>}
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.34 }}
                    className="bg-card border border-border rounded-2xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-border">
                        <h3 className="text-sm font-semibold text-foreground">Threats by Source</h3>
                    </div>
                    <div className="p-4 h-48">
                        {threatsBySource.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={threatsBySource} layout="vertical" margin={{ left: 0, right: 12 }}>
                                    ...
                                    <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 11 }} />
                                    <Bar dataKey="value" name="Alerts" radius={[0, 4, 4, 0]} maxBarSize={20}>
                                        {threatsBySource.map((_, i) => <Cell key={i} fill={["#06b6d4", "#8b5cf6", "#f59e0b", "#22c55e"][i % 4]} />)}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                            ...
                            <div key={item.label}>
                                ...
                                {item.pct > 0 && (
                                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                                        <motion.div className={cn("h-full rounded-full", item.color)} initial={{ width: 0 }} animate={{ width: `${item.pct}%` }} transition={{ duration: 0.7, delay: 0.4 }} />
                                    </div>
                                )}
                            </div>
                        ))}
            ...
            {topThreats.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.44 }}
                    className="bg-card border border-border rounded-2xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-border">
                        <h3 className="text-sm font-semibold text-foreground">Top Threat Categories</h3>
                    </div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <div className="flex-1 h-1 bg-muted rounded-full overflow-hidden">
                                            <div className="h-full bg-destructive rounded-full"
                                                style={{ width: `${topThreats.length > 0 ? (stats.count / topThreats[0][1].count) * 100 : 0}%` }} />
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 text-xs shrink-0">
                                    <span className="font-bold text-foreground">{stats.count} alerts</span>
                                    {stats.critical > 0 && <span className="font-bold text-destructive">{stats.critical} critical</span>}
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        { label: "Model Version", value: "v2.4.1", color: "text-green-500" },
                        { label: "Detection Accuracy", value: "99.9%", color: "text-green-500" },
                        { label: "Avg Inference", value: `${metrics.avgLatency}ms`, color: "text-primary" },
                        { label: "Detections (period)", value: metrics.total.toString(), color: "text-violet-500" },
                    ].map(s => (
