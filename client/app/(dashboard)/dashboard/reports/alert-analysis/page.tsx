...
export default function AlertAnalysisReport() {
  useEffect(() => {
    document.title = "Alert Analysis Report - EdgePulse";
  }, []);
  ...
  const filtered = useMemo(() => {
    return alerts.filter((a) => {
      ...
      if (selectedDevice !== "all" && a.device_id !== selectedDevice)
        return false;
      if (
        search &&
  ...
  const metrics = useMemo(
    () => ({
      ...
      resolutionRate:
        filtered.length > 0
          ? Math.round(
              (filtered.filter((a) => a.status === "CLOSED").length /
                filtered.length) *
                100,
            )
          : 0,
      avgLatency:
        filtered.length > 0
          ? Math.round(
              filtered.reduce((s, a) => s + a.inference_latency_ms, 0) /
                filtered.length,
            )
          : 0,
    }),
  ...
  const timeSeries = useMemo(() => {
    ...
    filtered.forEach((a) => {
      ...
      if (buckets[key])
        buckets[key][a.severity as keyof (typeof buckets)[string]]++;
    });
  ...
  const topDevices = useMemo(() => {
    const counts: Record<
      string,
      { name: string; count: number; critical: number }
    > = {};
    filtered.forEach((a) => {
      const name =
        devices.find((d) => d.id === a.device_id)?.name || a.device_id;
      if (!counts[a.device_id])
        counts[a.device_id] = { name, count: 0, critical: 0 };
  ...
  const handleExportAlertPDF = async (alert: (typeof filtered)[0]) => {
    try {
        ...
        criticalIncidents: [
          {
            ...
            description:
              typeof alert.explanation_json === "object"
                ? JSON.stringify(alert.explanation_json).substring(0, 200)
                : alert.title,
      ...
      a.href = url;
      a.download = `alert-${alert.id}-${now.toISOString().split("T")[0]}.pdf`;
      a.click();
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const chartImages = [];
      try {
        const trendsChart = await pdfService.captureChart(
          "alert-trends-chart",
      ...
      try {
        const severityChart = await pdfService.captureChart(
      ...
      try {
        const statusChart = await pdfService.captureChart(
      ...
      const reportData: ReportData = {
        ...
        alertTrends: timeSeries.map((t) => ({
          date: t.date,
          count: t.critical + t.high + t.medium + t.low,
        })),
        deviceRiskMatrix: topDevices.map((d) => ({
          ...
          status:
            d.critical > 0 ? "critical" : d.count > 10 ? "high" : "normal",
        })),
        distribution: {
          bySeverity: {
        ...
        criticalIncidents: filtered
          ...
          .map((a) => ({
            ...
            severity: a.severity,
            description:
              typeof a.explanation_json === "object"
                ? JSON.stringify(a.explanation_json).substring(0, 100)
                : a.title,
  ...
  const exportCSV = () => {
    try {
      ...
      const alertsWithShap = filtered.filter(
        (a) =>
          a.explanation_json &&
          typeof a.explanation_json === "object" &&
          "features" in a.explanation_json,
      );
      const shapFeatures = alertsWithShap.flatMap((a) => {
        const features =
          (
            a.explanation_json as {
              features?: Array<{
                feature_name: string;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Security alert trends, distribution, and performance metrics
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <SelectContent>
              ...
              {["PENDING", "ACKNOWLEDGED", "INVESTIGATED", "CLOSED"].map(
                (s) => (
          ...
          >
            ...
            <SelectContent>
              ...
              {["PROCESS", "NETWORK", "FILE", "RESOURCE"].map((s) => (
                <SelectItem key={s} value={s}>
                  {s}
                </SelectItem>
              ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        >
          ...
          <div
            ...
            className="p-4 h-56 flex items-center"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          ...
          <div
            ...
            className="p-4 h-52 flex items-center"
          >
            {statusDist.length > 0 ? (
              <>
                <div className="flex-1 h-full">
            ...
            <tbody className="divide-y divide-border">
              {filtered.slice(0, 100).map((a) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2.5 font-medium text-foreground max-w-50 truncate">
                      {a.title}
                    </td>
                    <td className="px-4 py-2.5 text-muted-foreground truncate max-w-30">
                      {devices.find((d) => d.id === a.device_id)?.name ||
                        a.device_id}
                    </td>
                    ...
                    <td className="px-4 py-2.5">
                      <span className="flex items-center gap-1 text-muted-foreground">
                        {telemetrySourceIcon(a.telemetry_source)}
                        {a.telemetry_source}
                      </span>
                    </td>
