...
class DeviceEnrollmentClient:
    ...
    def read_enrollment_config(self) -> Optional[EnrollmentConfig]:
        ...
        try:
            # Look for enrollment config in standard locations
            config_paths = []
            
            if self.platform == "win32":
                # Windows: ProgramData
                config_dir = Path(os.environ.get('ProgramData', 'C:\\ProgramData')) / 'EdgePulse'
    ...
    async def enroll_device(self, config: EnrollmentConfig) -> Optional[EnrollmentResponse]:
        ...
        try:
            ...
            agent_version = config.agent_version or self._get_agent_version()
            ...
            async with httpx.AsyncClient(timeout=config.timeout_seconds) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Device enrolled successfully: {enrollment_response.device_id}")
    ...
    def complete_enrollment(self, response: EnrollmentResponse) -> bool:
        ...
        try:
            ...
            if success:
                # Clear enrollment token from storage
    ...
    async def rotate_api_key(self, supabase_url: str) -> Optional[str]:
        ...
        try:
            ...
            # Call rotate API key edge function
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code == 200:
                    ...
                    # Store new API key
                    success = self.credential_manager.store_credential('api_key', new_api_key)
    ...
    async def verify_enrollment(self, supabase_url: str) -> bool:
        ...
        try:
            ...
            # Simple health check to verify credentials work
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                response = await client.get(
                    ...
                    headers={
                        "X-EdgePulse-Device-Id": credentials.device_id,
                        "X-EdgePulse-Api-Key": credentials.api_key,
                        "apikey": "dummy",  # Supabase requires this even if not used
                        "Authorization": "Bearer dummy"
                    }
                )
                
                # We expect this to fail with 401 if auth is working properly
                # but the service should be reachable
