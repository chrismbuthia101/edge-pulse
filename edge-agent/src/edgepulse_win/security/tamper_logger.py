...
class TamperEvidentLogger:
    ...
    def __init__(self, device_id: str, db_manager: DatabaseManager):
        ...
        # Device signing key (generated on first run)
    ...
    async def _load_state(self):
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"Loaded tamper log state: sequence {self.current_sequence}")
            else:
    ...
    def _compute_content_hash(self, entry_data: Dict[str, Any]) -> str:
        ...
        # Create content dict without hash fields
        content = {
    ...
    async def log_event(self, 
        ...
        try:
            ...
            if success:
                ...
                logger.debug(f"Logged tamper event: {entry_type} (sequence {self.current_sequence})")
                return True
            else:
    ...
    async def _store_entry(self, entry: TamperEntry) -> bool:
        ...
        try:
            ...
            params = (
                entry.log_id, entry.device_id, entry.log_sequence_number,
                entry.log_entry_type, entry.log_entry_reference_id,
                entry.entry_timestamp_utc, entry.entry_content_hash,
                entry.previous_entry_hash, entry.digital_signature
            )
    ...
    async def verify_chain(self, 
                       ...
                       to_sequence: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, entry in enumerate(entries):
                ...
                if i > 0 and entry['previous_entry_hash'] != expected_hash:
                    ...
                    break_reason = f"Hash chain broken at sequence {entry['log_sequence_number']}"
                    break
                
                # Verify content hash if signature present
                if entry['digital_signature']:
                    signature_valid = await self._verify_signature(entry)
                    if not signature_valid:
                        first_broken = entry['log_sequence_number']
                        break_reason = f"Invalid signature at sequence {entry['log_sequence_number']}"
                        break
    ...
    async def get_chain_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = await self.db_manager.execute_query(stats_query, (self.device_id,))
            ...
            recent = await self.db_manager.execute_query(recent_query, (self.device_id,))
    ...
    async def export_chain(self, 
                        from_sequence: Optional[int] = None,
                        to_sequence: Optional[int] = None) -> str:
        ...
        try:
            ...
            export_data = {
                ...
                'entries': entries,
                'verification': await self.verify_chain(from_sequence, to_sequence)
            }
    ...
    async def cleanup_old_entries(self, days_to_keep: int = 90):
...
# Factory function
def create_tamper_logger(device_id: str, db_manager: DatabaseManager) -> TamperEvidentLogger:
