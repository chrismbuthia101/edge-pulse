"""Generates a bootstrapped Isolation Forest model matching feature_extractor.py feature schema."""
...
def generate_synthetic_normal(n_samples: int, rng: np.random.Generator) -> np.ndarray:
    ...
    data[:, IDX["process_cpu_gini_1min"]] = rng.beta(1.5, 5, n_samples)
    data[:, IDX["process_admin_ratio_1min"]] = rng.beta(1, 9, n_samples)
    data[:, IDX["process_no_exe_path_ratio_1min"]] = rng.beta(1, 19, n_samples)
    data[:, IDX["process_long_cmdline_ratio_1min"]] = rng.beta(1, 19, n_samples)
