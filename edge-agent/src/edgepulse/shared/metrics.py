...
class MetricCollector(ABC):
    ...
    def increment_counter(
        self, metric_def: MetricDefinition, value: int = 1, labels: Optional[Dict[str, str]] = None
    ) -> None:
        pass
    ...
    def observe_histogram(
        self, metric_def: MetricDefinition, value: float, labels: Optional[Dict[str, str]] = None
    ) -> None:
        pass
...
class InMemoryMetricsCollector(MetricCollector):
    ...
    def record_anomaly(self, severity: str = "medium") -> None:
        self.increment_counter(
            StandardMetrics.ANOMALIES_DETECTED_TOTAL, labels={"severity": severity}
        )
    ...
    def record_alert(
        ...
        self.increment_counter(
            StandardMetrics.ALERTS_GENERATED_TOTAL, labels={"severity": severity}
        )

        if anomaly_score is not None:
            self.observe_histogram(
                StandardMetrics.ALERT_ANOMALY_SCORE, anomaly_score, labels={"severity": severity}
            )
