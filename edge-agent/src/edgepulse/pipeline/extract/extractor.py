...
from edgepulse.models import normalize_timestamp
from edgepulse.pipeline.extract.cpu import CpuFeatureExtractor
from edgepulse.pipeline.extract.memory import MemoryFeatureExtractor
from edgepulse.pipeline.extract.disk import DiskFeatureExtractor
from edgepulse.pipeline.extract.network import NetworkFeatureExtractor
from edgepulse.pipeline.extract.process import ProcessFeatureExtractor
...
class FeatureExtractor:
    def __init__(
        ...
        if not all(isinstance(w, int) and w > 0 for w in (window_1min, window_5min)):
