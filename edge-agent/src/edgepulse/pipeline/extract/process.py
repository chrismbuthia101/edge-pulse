    ...
    def extract(self, processes: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "process_admin_ratio_1min": (
                admin_count / total_processes if total_processes > 0 else 0.0
            ),
            "process_no_exe_path_ratio_1min": (
                no_exe_path_count / total_processes if total_processes > 0 else 0.0
            ),
            "process_long_cmdline_ratio_1min": (
                long_cmdline_count / total_processes if total_processes > 0 else 0.0
            ),
        }
