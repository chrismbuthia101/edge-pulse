...
class DeviceNormalizer:
    ...
    def fit(self, features: np.ndarray) -> None:
        ...
        try:
            ...
            if isinstance(self.scaler, RobustScaler):
                self.baseline_mean = (
                    self.scaler.center_.copy() if self.scaler.center_ is not None else None
                )
                self.baseline_std = (
                    self.scaler.scale_.copy() if self.scaler.scale_ is not None else None
                )
            elif isinstance(self.scaler, StandardScaler):
                self.baseline_mean = (
                    self.scaler.mean_.copy() if self.scaler.mean_ is not None else None
                )
                self.baseline_std = (
                    self.scaler.scale_.copy() if self.scaler.scale_ is not None else None
                )
    ...
    def load_baseline(self, path: Optional[Path] = None) -> bool:
        ...
        if load_path is None:
            logger.warning(
                "Baseline file not found in any location",
