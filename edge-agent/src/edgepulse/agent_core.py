...
from edgepulse.utils.log_handler import get_logger, ConfigurationError
from edgepulse.utils.version import get_agent_version
from edgepulse.detectors.model_manager import ModelManager
from edgepulse.core.enrollment_manager import EnrollmentManager
...
class AgentCore:
    ...
    async def _init_credential_manager(self) -> None:
        ...
        try:
            ...
            if not self.credential_manager.is_enrolled():
                logger.warning(
                    "Device not yet enrolled – EnrollmentManager will attempt enrollment"
                )
            else:
    ...
    async def _init_enrollment_manager(self) -> None:
        """Initialize enrollment manager and check/perform enrollment."""
        try:
    ...
    async def _handle_alert(
        ...
        try:
            ...
            if explanation is not None:
                explanation_json_str = explanation.to_json()
            else:
