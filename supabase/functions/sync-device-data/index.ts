...
const VALID_TELEMETRY_SOURCES = new Set(['PROCESS', 'NETWORK', 'FILE', 'RESOURCE'])
    ...
    if (syncData.alerts && syncData.alerts.length > 0) {
      handlers.push((async () => {
        ...
        try {
          ...
          const exclude = new Set(['device_id', 'organization_id', 'created_at', 'updated_at'])
          const { valid: validAlerts, failedCount: failedAlerts } = await filterValidRecords(
            integrityKey, batch as unknown as Record<string, unknown>[],
            exclude,
    ...
    if (syncData.telemetry && syncData.telemetry.length > 0) {
      handlers.push((async () => {
        try {
          ...
          const exclude = new Set(['device_id', 'organization_id', 'received_at'])
          const { valid: validEvents, failedCount: failedEvents } = await filterValidRecords(
            integrityKey, batch as unknown as Record<string, unknown>[],
            exclude,
          ) as unknown as { valid: typeof batch; failedCount: number }
          ...
          const telemetry = validEvents.map(event => ({
            ...
            source: VALID_TELEMETRY_SOURCES.has(event.source) ? event.source : 'RESOURCE',
            payload: event.payload,
            connectivity_state: event.connectivity_state === 'offline' ? 'offline' : 'online',
    ...
    if (syncData.heartbeat) {
      handlers.push((async () => {
        try {
          ...
          if (hb.cpu_percent !== undefined) updateFields.cpu_percent = hb.cpu_percent
          if (hb.ram_percent !== undefined) updateFields.ram_percent = hb.ram_percent
          if (hb.sync_queue_depth !== undefined) updateFields.sync_queue_depth = hb.sync_queue_depth
          if (hb.alerts_count !== undefined) updateFields.alerts_count = hb.alerts_count
          if (hb.agent_version !== undefined) updateFields.agent_version = hb.agent_version
    ...
    if (syncData.health_snapshots && syncData.health_snapshots.length > 0) {
      handlers.push((async () => {
        try {
          ...
          const exclude = new Set(['device_id', 'organization_id', 'created_at'])
          const { valid: validSnapshots, failedCount: failedSnapshots } = await filterValidRecords(
            integrityKey, batch as unknown as Record<string, unknown>[],
            exclude,
          ) as unknown as { valid: typeof batch; failedCount: number }
          if (failedSnapshots > 0) response.health_snapshots_failed += failedSnapshots
    ...
    if (syncData.anomaly_scores && syncData.anomaly_scores.length > 0) {
      handlers.push((async () => {
        try {
          ...
          const exclude = new Set(['device_id', 'organization_id', 'created_at'])
          const { valid: validScores, failedCount: failedScores } = await filterValidRecords(
            integrityKey, batch as unknown as Record<string, unknown>[],
            exclude,
          ) as unknown as { valid: typeof batch; failedCount: number }
          if (failedScores > 0) response.anomaly_scores_failed += failedScores
          const scores = validScores.map(s => ({
            ...
            connectivity_state: s.connectivity_state === 'offline' ? 'offline' : 'online',
    ...
    if (syncData.feature_vectors && syncData.feature_vectors.length > 0) {
      handlers.push((async () => {
        try {
          ...
          const exclude = new Set(['device_id', 'organization_id', 'created_at', 'received_at'])
          const { valid: validFvs, failedCount: failedFvs } = await filterValidRecords(
            integrityKey, batch as unknown as Record<string, unknown>[],
            exclude,
          ) as unknown as { valid: typeof batch; failedCount: number }
...
function canonicalJson(record: Record<string, unknown>, exclude: Set<string>): string {
...
async function computeIntegrityHash(key: CryptoKey, record: Record<string, unknown>, exclude: Set<string>): Promise<string> {
...
async function filterValidRecords<T extends Record<string, unknown>>(
  key: CryptoKey, records: T[], exclude: Set<string>,
