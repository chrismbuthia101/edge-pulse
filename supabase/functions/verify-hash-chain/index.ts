// EdgePulse Verify Hash Chain Function v3.0.0
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (fromSequence) query = query.gte('sequence_number', parseInt(fromSequence))
      if (toSequence) query = query.lte('sequence_number', parseInt(toSequence))
  ...
  for (const entry of entries) {
    if (entry.previous_hash !== previousHash) {
      return {
        ...
        break_reason: `Hash chain broken at sequence ${entry.sequence_number}: expected ${previousHash}, got ${entry.previous_hash}`,
      }
    }

    const expectedContentHash = await computeContentHash(entry)
    if (entry.content_hash !== expectedContentHash) {
      return {
        is_valid: false,
        first_broken_sequence: entry.sequence_number,
        break_reason: `Content hash mismatch at sequence ${entry.sequence_number}: expected ${expectedContentHash}, got ${entry.content_hash}`,
      }
    }
    ...
    if (entry.digital_signature) {
      ...
      if (!signatureValid) {
        return {
          is_valid: false,
          first_broken_sequence: entry.sequence_number,
          break_reason: `Invalid digital signature at sequence ${entry.sequence_number}`,
        }
      }
    }
