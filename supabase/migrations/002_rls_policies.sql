...
CREATE POLICY "platform_admins: manage organizations" ON organization.organizations FOR ALL USING (
    (
        SELECT internal.is_platform_admin ()
    )
);

CREATE POLICY "org_admins: view own organization" ON organization.organizations FOR
SELECT USING (
        (
...
CREATE POLICY "platform_admins: manage billing" ON organization.billing FOR ALL USING (
    (
        SELECT internal.is_platform_admin ()
    )
);

CREATE POLICY "org_admins: view own billing" ON organization.billing FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── users ───────────────────────────────────────────────────────────────────
CREATE POLICY "org_admins: view organization users" ON public.users FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
CREATE POLICY "users: view self" ON public.users FOR
SELECT USING (
        id = (
            SELECT auth.uid ()
        )
...
CREATE POLICY "platform_admins: create platform users" ON public.users FOR INSERT
WITH
    CHECK (
        (
            SELECT internal.is_platform_admin ()
...
CREATE POLICY "org_admins: manage organization users" ON public.users FOR
UPDATE USING (
    (
        SELECT internal.is_org_admin ()
    )
...
SELECT USING (
        id = (
            SELECT internal.current_device_id ()
        )
    );

CREATE POLICY "org_admins: manage organization devices" ON public.devices FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
CREATE POLICY "devices: update own" ON public.devices FOR
UPDATE USING (
    id = (
        SELECT internal.current_device_id ()
    )
...
-- ── api_keys ────────────────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization api keys" ON devices.api_keys FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── enrollment_tokens ──────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization enrollment tokens" ON devices.enrollment_tokens FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── device_assignments ─────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization assignments" ON public.device_assignments FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
CREATE POLICY "org_analysts: view own assignments" ON public.device_assignments FOR
SELECT USING (
        user_id = (
            SELECT auth.uid ()
        )
...
-- ── telemetry events ───────────────────────────────────────────────────────
CREATE POLICY "devices: insert own events" ON telemetry.events FOR INSERT
WITH
    CHECK (
        device_id = (
            SELECT internal.current_device_id ()
...
CREATE POLICY "org_admins: read organization events" ON telemetry.events FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── feature_vectors ────────────────────────────────────────────────────────
CREATE POLICY "devices: insert own feature vectors" ON telemetry.feature_vectors FOR INSERT
WITH
    CHECK (
        device_id = (
            SELECT internal.current_device_id ()
...
CREATE POLICY "org_admins: read organization feature vectors" ON telemetry.feature_vectors FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── anomaly_scores ─────────────────────────────────────────────────────────
CREATE POLICY "devices: insert own anomaly scores" ON telemetry.anomaly_scores FOR INSERT
WITH
    CHECK (
        device_id = (
            SELECT internal.current_device_id ()
...
CREATE POLICY "org_admins: read organization anomaly scores" ON telemetry.anomaly_scores FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── alerts ─────────────────────────────────────────────────────────────────
CREATE POLICY "devices: insert own alerts" ON public.alerts FOR INSERT
WITH
    CHECK (
        device_id = (
            SELECT internal.current_device_id ()
...
CREATE POLICY "org_admins: manage organization alerts" ON public.alerts FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── device_health ──────────────────────────────────────────────────────────
CREATE POLICY "devices: insert own health" ON telemetry.device_health FOR INSERT
WITH
    CHECK (
        device_id = (
            SELECT internal.current_device_id ()
...
CREATE POLICY "org_admins: read organization device health" ON telemetry.device_health FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── sync_queue ─────────────────────────────────────────────────────────────
CREATE POLICY "devices: manage own sync queue" ON internal.sync_queue FOR ALL USING (
    device_id = (
        SELECT internal.current_device_id ()
    )
);

CREATE POLICY "org_admins: read organization sync queue" ON internal.sync_queue FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
-- ── retention_settings ─────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization retention" ON public.retention_settings FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── privacy_settings ──────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization privacy settings" ON public.privacy_settings FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── config ─────────────────────────────────────────────────────────────────
CREATE POLICY "devices: read own config" ON devices.config FOR
SELECT USING (
        device_id = (
            SELECT internal.current_device_id ()
        )
...
CREATE POLICY "org_admins: manage organization device config" ON devices.config FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
-- ── models ─────────────────────────────────────────────────────────────────
CREATE POLICY "org_admins: manage organization models" ON internal.models FOR ALL USING (
    (
        SELECT internal.is_org_admin ()
    )
    AND organization_id = (
...
CREATE POLICY "org_members: read organization models" ON internal.models FOR
SELECT USING (
        organization_id = (
            SELECT internal.current_organization_id ()
        )
...
CREATE POLICY "platform_admins: manage all models" ON internal.models FOR ALL USING (
    (
        SELECT internal.is_platform_admin ()
    )
);
...
CREATE POLICY "org_admins: read organization notifications" ON public.notifications FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
CREATE POLICY "service: insert audit logs" ON internal.audit_logs FOR INSERT
WITH
    CHECK (FALSE);

CREATE POLICY "org_admins: read organization audit logs" ON internal.audit_logs FOR
SELECT USING (
        (
            SELECT internal.is_org_admin ()
        )
...
CREATE POLICY "platform_admins: read all audit logs" ON internal.audit_logs FOR
SELECT USING (
        (
            SELECT internal.is_platform_admin ()
        )
...
GRANT
SELECT, INSERT,
UPDATE ON public.device_assignments TO authenticated;

GRANT SELECT ON internal.audit_logs TO authenticated;

GRANT
SELECT, INSERT,
UPDATE ON public.privacy_settings TO authenticated;

GRANT
SELECT, INSERT,
UPDATE, DELETE ON internal.sync_queue TO authenticated;

GRANT
SELECT, INSERT,
UPDATE ON public.retention_settings TO authenticated;
...
REVOKE EXECUTE ON FUNCTION internal.sync_organization_to_jwt ()
FROM PUBLIC;

REVOKE EXECUTE ON FUNCTION internal.current_organization_id ()
FROM PUBLIC;
...
REVOKE EXECUTE ON FUNCTION internal.get_device_id_from_headers ()
FROM PUBLIC;

REVOKE EXECUTE ON FUNCTION internal.get_api_key_from_headers ()
FROM PUBLIC;
